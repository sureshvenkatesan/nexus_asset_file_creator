...
def convert_csv_to_json(csv_file, json_file):
    ...
    with open(csv_file, 'r') as file:
        ...
        for row in reader:
            # Construct the 'source' path with '/' replacing '.' in the Group field
            group_path = row['Group'].replace('.', '/')
            source_path = f"{row['Repository']}/{group_path}/{row['Artifact']}/{row['Version']}/{row['file']}"
